...
int idTimer = -1, idTimer2 = -1, doTempAnti;
int n;
int ctr = 0;
double scale, speed, fps;
...
char* averageFrames(char* f1, char* f2, char* f3, int w, int h) {
	char* imgData = new char[w * h * 3];

	for (int i = 0; i < w * h; ++i)
	{
		imgData[3 * i] = (f1[3 * i] + f2[3 * i] + f3[3 * i])/3;
		imgData[3 * i + 1] = (f1[3 * i + 1] + f2[3 * i + 1] + f3[3 * i + 1]) / 3;
		imgData[3 * i + 2] = (f1[3 * i + 2] + f2[3 * i + 2] + f3[3 * i + 2]) / 3;
	}

	return imgData;
}

void draw(int n, double scale, int delta, bool renderOp = true) {
	// Set up the images
	int w = 512;
	int h = 512;

	char* imgData = getBuff(delta);

	char* toDel = inImage.getImageData();
	if (toDel != NULL) {
		delete toDel;
	}
	...
	if (renderOp) {
		...
		char* oimgData = NULL;
		if (doTempAnti) {
			double d = (speed * 360) / 60;
			char *oimgData1, *oimgData2, *oimgData3;
			oimgData1 = resizeWithAliasing(imgData, w, h, scale);
			char* tmp = getBuff(delta - d);
			char* tmp2 = getBuff(delta + d);
			oimgData2 = resizeWithAliasing(tmp, w, h, scale);
			oimgData3 = resizeWithAliasing(tmp2, w, h, scale);
			oimgData = averageFrames(oimgData1, oimgData2, oimgData3, w, h);
			delete oimgData1;
			delete oimgData2;
			delete oimgData3;
			delete tmp;
			...
			oimgData = resizeWithAliasing(imgData, w, h, scale);
		}

		toDel = outImage.getImageData();
		if (toDel != NULL) {
			delete toDel;
		}

		outImage.setImageData(oimgData);
	}
                     ...
                     int       nCmdShow)
{
	...
	sscanf(lpCmdLine, "%d %lf %lf %d %lf", &n, &speed, &fps, &doTempAnti, &scale);
	...
	printf("The fourth parameter was: %d\n", fps);
...
// You are free to change the following code in any way in order to display the video
	...
	switch (message) 
	{
		...
		case WM_CREATE: {
			...
			double elapsedTime = elapsed.count() / 15.0;
			double timer = 1.0 / fps;

			if (timer * 1000 - elapsedTime > 0) {
				SetTimer(hWnd, 1, timer * 1000 - elapsedTime, NULL);
			} else {
				SetTimer(hWnd, 1, timer * 1000, NULL);
			}
			SetTimer(hWnd, 2, (1/60) * 1000, NULL);
			break;
		}
		case WM_TIMER: {
			double d = (speed * 360) / 60;
