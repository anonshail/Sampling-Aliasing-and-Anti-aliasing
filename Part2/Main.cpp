...
void draw(int n, double scale, int delta, bool renderOp = true) {
	...
	if (renderOp) {
		outImage = inImage;
		char* oimgData = resize(imgData, w, h, scale);
		//char* oimgData = imgData;

		toDel = outImage.getImageData();
		outImage.setImageData(NULL);
		if (toDel != NULL) {
			delete toDel;
		}
                     ...
                     int       nCmdShow)
{
	...
	sscanf(lpCmdLine, "%d %lf %lf", &n, &speed, &fps);
...
// You are free to change the following code in any way in order to display the video
	...
	switch (message) 
	{
		...
		case WM_CREATE: {
			...
			double elapsedTime = elapsed.count() / 5.0;
